...
annotations_path = data_path + "/annotations/train.json"
...
for (window_np, gt_boxes, gt_classes) in preprocessor.iterate(win_set):
    #Image.fromarray(window_np).show()
    ...
    for (cls_targets, cls_weights, reg_targets, reg_weights, matched) in zip(*targets_set):
        pass
        # Write them as a TF record
