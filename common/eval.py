...
# Temporary
from ood.evaluate import COCOeval
from pycocotools.coco import COCO

def coco_eval(model, annotations_path, image_dir, desired_categories,
        win_set=None,
        min_coverage=.3,
        results_dir="/tmp",
        agnostic_mode=False):
    ...
    cocoEval = COCOEvalWrapper(cocoGt, cocoDt, iou_type="bbox", agnostic_mode=agnostic_mode)
...
def coco_by_category(model, annotations_path, image_dir, desired_categories,
        win_set=None,
        min_coverage=.3,
        results_dir="/tmp"):
    timestamp   = int(time())
